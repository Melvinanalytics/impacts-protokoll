...
| Actual work and evidence | Reached attempt `input/` and `output/`; only `laufpfad` owns run state |
...
Answers name the correct entities and relationships, evidence status, source and every read path. They invent no claims and create no second fact home. Repair unclear definitions, boundaries or inaccessible existing evidence at their home or link. Truly missing evidence stays a focused question until supported; a new summary or runtime cannot settle it.
