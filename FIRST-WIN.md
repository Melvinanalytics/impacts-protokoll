...
Follow the [method](02_protocol/impacts-method.md) and [Run template](02_protocol/templates/vorgang.md) for those steps. Do not invent a tree ID, hash, execution entry or approval. Resume from the preserved draft when tooling becomes available. Structural validation alone proves neither executed work nor a checked business result; required domain checks and human decisions still need their own evidence.
