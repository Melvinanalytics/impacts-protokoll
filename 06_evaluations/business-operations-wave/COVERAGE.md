...
| v0.3.6 | Fresh semantic first pass, 64 cases | not yet run |
| v0.3.6 | Independent adversarial pass | not yet run |

The v0.3.6 entries include only the three new Markdown files and their router link on top of the named tag; they establish regression behavior for that working tree, not a released revision. `Not yet run` means no independent result has been produced. Update this table only with command, exact revision, environment and observed output; do not infer a pass from a nearby tree.
