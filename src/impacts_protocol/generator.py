...
2. Application entwerfen: `impacts template <art>` liefert jede `CONTEXT.md` (application, hauptprozess, teilprozess, arbeitsschritt). Der Application-Baum enthält nur `CONTEXT.md`-Dateien. Nach dem Commit liefert `git rev-parse HEAD:applications/<slug>` die Revision. Eine Application aus einem anderen Repository kommt als Byte-Kopie von `applications/<slug>/` (`git archive | tar -x`), committet mit Quellcommit und Tree-OID in der Commit-Nachricht; der OID ist auf beiden Seiten gleich.
3. Vorgang öffnen: `vorgaenge/<slug>/CONTEXT.md` aus `impacts template vorgang` mit `application_revision: git-tree:<oid>`. Der erste Laufpfadeintrag ist der Einstieg der Application, Versuch `001`, Status `aktiv`. Eingaben liegen unter `arbeitsschritte/<schritt>/001/input/`; `impacts hash <versuchsordner> <flaeche>...` liefert `eingabe_hash`.
