...
Die acht Regeln, jede mit Primärquelle:
...
8. **Lokaler Pfad vor neuer Ingestion.** Bei bereits vorhandenem Wissen führt der Abruf vom nächsten `CONTEXT.md` über bestehende Markdown-Links zur Fachdatei und ihrer Quelle. Ist ein lokaler Fakt nicht erreichbar, wird sein Router oder Link an der einzigen Faktenheimat repariert. Eine neue Zusammenfassung oder Webrecherche ist kein Ersatz. Externe Recherche beginnt erst bei einer belegten Evidenzlücke, ausdrücklich verlangter Aktualisierung oder ausdrücklichem Nutzerauftrag. (ICM: Layered Loading, Edit-Source Principle und "one home per fact")
